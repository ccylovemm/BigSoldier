...
public class MadLevelGridLayout : MadLevelAbstractLayout {
    ...
    [HideInInspector] [NonSerialized] public bool hidePrevSlideIcon;
    [HideInInspector] [NonSerialized] public bool hideNextSlideIcon;
    ...
    private void UpdateLayout(bool forceDelete) {
        if (IsDirty()) {
            bool deep = deepClean || (!Application.isPlaying && (forceDelete || generate));
            CleanUp(deep);
            Build(deep);
            MakeClean();
